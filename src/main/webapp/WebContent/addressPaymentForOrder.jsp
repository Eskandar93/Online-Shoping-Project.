...
%>
        ...
        <tbody>
<%
	ResultSet rs2 = st.executeQuery("select *from product inner join cart on product.id=cart.product_id and cart.email='"+email+"' and address is NULL ");
	while(rs2.next()){
%>        
          <tr>
          <%sno +=1; %>
           <td><%out.println(sno); %></td>
            ...
            <td><i class="fa fa-inr"></i> <%=rs2.getInt(4) %></td>
            <td> <%=rs2.getInt(8) %></td>
            <td><i class="fa fa-inr"></i> <%=rs2.getDouble(10) %></td>
            </tr>
   <%}
